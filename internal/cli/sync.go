...
`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Workspace names are sanitized to alphanumerics before use, so two
			// distinct source names can collapse onto the same target.
			if to == "keyharbour" && targetWorkspaceRef == "" {
				names := make([]string, 0, len(objs))
				for _, obj := range objs {
					if obj.Workspace != "" {
						names = append(names, obj.Workspace)
					}
				}
				if err := checkWorkspaceNameCollisions(names); err != nil {
					return err
				}
			}
			...
			results := workerpool.RunContext(ctx, objs, concurrency, func(obj backend.Object) error {
				...
				case "keyharbour":
					...
					if targetWorkspaceName == "" {
						...
						targetWorkspaceName, err = validateAndSanitizeWorkspaceName(targetWorkspaceName, cmd.ErrOrStderr())
						if err != nil {
							return err
						}
					}
