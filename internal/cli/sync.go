			...
			if genBackend && to == "keyharbour" && destProj != nil {
				...
				if wsName == "" {
					...
					if err != nil {
						...
						fmt.Fprintf(cmd.OutOrStdout(), "  Add to .gitignore — file contains your API token\n")
...
func writeBackendSample(outPath, endpoint, workspaceUUID, token string) error {
...
`, time.Now().Format("2006-01-02"), stateAddr, lockAddr, lockAddr, token)
